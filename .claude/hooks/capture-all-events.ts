 ...
 * This hook provides comprehensive event tracking for observability
 *
 * SETUP REQUIRED:
 ...
 * SANITIZED VERSION - Configure for your needs
 */
...
// Get formatted timestamp (adjust timezone as needed for your location)
function getFormattedTimestamp(): string {
  const date = new Date();
  // Change timezone to your preferred one
...
function getAgentForSession(sessionId: string): string {
  try {
    ...
    if (existsSync(mappingFile)) {
      ...
      // Default agent name - change 'assistant' to your primary agent's name
      return mappings[sessionId] || 'assistant';
    }
  } catch (error) {
    // Ignore errors, default to primary agent
  }
  // Change 'assistant' to your primary agent's name
  return 'assistant';
}
...
async function main() {
  try {
    ...
    // If this is a SubagentStop or Stop event, reset to primary agent
      ...
      // Change 'assistant' to your primary agent's name
      agentName = 'assistant';
      setAgentForSession(sessionId, 'assistant');
    }
    ...
    const event: HookEvent = {
      ...
      payload: hookData,
      timestamp: Date.now(),
      timestamp_formatted: getFormattedTimestamp()
    };
